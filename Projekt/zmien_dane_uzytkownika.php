  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="row">
        ...
        $stmtuzytkownik = $pdo->query("SELECT ID, imie, nazwisko FROM osoby WHERE UPRAWNIENIE != 'Administrator'");
		...
		<form method="post">
			...
			if (isset($_POST['pokaz'])) {
                ...
                $stmt = $pdo -> query ('SET NAMES utf8');
				$stmt = $pdo->query('SELECT * from osoby WHERE osoby.ID='.$_POST['uzytkownik'].'');
				
				foreach($stmt as $row){
					echo '<form method="post" action="zmien_dane_uzytkownika_2.php">';
	  ...
	  echo '<input type="hidden" name="edytuj_ktory" value="'.$_POST['uzytkownik'].'" />';
			}
		?>
