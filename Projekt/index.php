      ...
      <div class="row">
        ...
        <div class="col-lg-3">
          ...
          <div class="list-group">
              ...
              {
                   echo '<form  method="get" class="list-group-item"><input type="hidden" name="c1" value="'.$row['nazwa'].'"> <input style="width: 220px; height: 25px; background-color: white; color: black; border: 0px solid #e7e7e7;" type="submit" value="'.$row['nazwa'].'" ></form>';
              }
              ?>
          </div>
            ...
            <div id="demo" class="list-group">
              <h1 class="my-4">Producent</h1>
                  ...
                  {
                      echo '<form  method="get" class="list-group-item" style=""><input type="hidden" name="c2" value="'.$row['nazwa'].'"> <input style="width: 220px; height: 25px; background-color: white; color: black; border: 0px solid #e7e7e7;" type="submit" value="'.$row['nazwa'].'" ></form>';
                  }
                  ?>
              </div>
            <br>
        ...
        <div class="col-lg-9">
          ...
          <div class="row">
			...
			//if ($_SESSION['szukaj_producent'] != null){
				...
				//if ($_SESSION['szukaj_producent'] == $row['PRODUCENT'];
			...
			//}
              ...
              foreach($stmt3 as $row)
              {
                  ...
                  echo '<a><img class="card-img-top" src="/img/products/'.$row['MODEL'].'.jpg" alt=""></a>';
                  echo '<div class="card-body">';
                  echo '<h4 class="card-title">';
                  echo '<a>'.$row['PRODUCENT'].' '.$row['MODEL'].'</a>';
                  echo '</h4>';
                  echo '<h5></h5>';
                  ...
                  if(@$_SESSION['zalogowany']==true){
                      ...
                      if (isset($_POST['transakcja'.$row['ID']])) {
                          $_SESSION['produkt'.$row['ID'].''] = array($row['PRODUCENT'], $row['MODEL'], $row['OPIS'], $row['CENA'], $row['ID'], $_POST['ilosc']);
                          echo "<script language='javascript'>alert('Dodano do koszyka.');</script>";
                      }
                  }

                  echo '<br><form method="post" style="display:inline;">';
                  echo '<br><input type="submit" style="width: 100%; height: 25px; background-color: white; color: black; border: 0px solid #e7e7e7;" name="szczegoly'.$row['ID'].'" value="Wiecej" /></form>';

                  if (isset($_POST['szczegoly'.$row['ID']])) {
                      $_SESSION['szczegol'.$row['ID']] = array($row['PRODUCENT'], $row['MODEL'], $row['OPIS'], $row['CENA'],$row['TYP'], $row['ID']);
                      echo "<script>

                    setTimeout(function () {
                       window.location.href= 'szczegoly.php'; // the redirect goes here
