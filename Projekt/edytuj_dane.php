  ...
  <body>
    ...
    <div class="container">
      ...
      <div class="row">
		...
		<?php
            echo'<a href="uzytkownik.php"> Wróć </a><br><br><br>';
			
			@$id = $_SESSION['id_uzytkownik'];
            $stmt = $pdo -> query ('SET NAMES utf8');
			$stmt = $pdo->query('SELECT * from osoby WHERE osoby.ID='.$id.'');
	  foreach($stmt as $row)
      {
		  echo '<form method="post" action="edytuj_dane_2.php">';
		  echo 'Login: <input type="text" name="login" value="'.$row['LOGIN'].'" /><br><br>';
		  echo 'Hasło: <input type="password" name="haslo" value="'.md5($row['HASLO']).'" /><br><br><br>';
	  ...
	  echo '<input type="hidden" name="edytuj_ktory" value="'.$id.'" />';
